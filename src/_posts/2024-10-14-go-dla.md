...
minimal length.  Let's call $\mathcal{S}_n$ the set of all steps in $n$
dimensions.  For example

* $\mathcal{S}_1 = \{-1, 1\}$
* $\mathcal{S}_2 = \{(0, -1), (0, 1), (-1, 0), (1, 0)\}$

A **walk** in $n$ dimensions is simply a sequence of steps in $n$ dimensions.
Given Let's call $\mathcal{W}_n^N$ the set of all walks in $n$ dimenstions of
length $N$.  We will call such walks **$N$-walks in $n$ dimensions**. It is
evident that

$$\mathcal{W}_n^N \equiv (\mathcal{S}_n)^N$$

Now given a walk $w \in \mathcal{W}_n^N, w = (\mathbf{s}_i)_{i < N}$, let's call
the **destination** $\delta(w)$ of $w$ the vector in $\mathbb{Z}^N$ which is the
sum of all the steps that compose $w$.  In mathematical notation that means

$$\delta(w) = \sum_{i < N} \mathbf{s}_i$$

Finally, given a vector $\mathbf{u} \in \mathbb{Z}^n$ we will call
$\mathcal{W}_{\mathbf{u}}^N$ the set of $N$-walks with destination $\mathbf{u}$.

$$\mathcal{W}_{\mathbf{u}}^N = \{ w \in \mathcal{W}_n^N | \delta(w) = \mathbf{u} \}$$

So for example
- $\mathcal{W}_1^3 = \{ \langle 1, 1, -1\rangle, \langle 1, -1, 1\rangle, \langle -1, 1, 1\rangle \}$
- $\mathcal{W}_2^3 = \emptyset$
- $\mathcal{W}_{1, 1}^2 = \{ \langle(1, 0), (0, 1)\rangle, \langle(0, 1), (1, 0)\rangle \}$

Our purpose is to calculate the cardinality of $\mathcal{W}_{x, y}^N$ for any,
$x, y \in\mathbb{Z}$.  This way we should be able to draw at random a walk in 2
dimensions with $N$ steps. So let's define

$$W_\mathbf{u}^N = \left|\mathcal{W}_\mathbf{u}^N\right|$$

In the rest of this section we will try to calculate $W_\mathbf{u}^N$ for any
$\mathbf{u}\in\mathbb{Z}^n$.

# Combinatorics of 1-walks

Given $N > 1$ and $w \in \mathcal{W}^N$, there is $w^* \in \mathcal{W}_1^{N-1}$
and $\mathbf{s}\in\mathcal{S}_1$ such that $w = \langle w^*,
\mathbf{s}\rangle$. 

We can establish naturally a mapping
